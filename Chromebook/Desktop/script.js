...
async function main() {
    ...
    let appChrome = new (InstalledModules.get("Chrome")).Chrome();
    appChrome.InstallApp();

    // Built-in console
    await loadApp("./built-ins/console.js")
    console.log(InstalledModules);
    let devConsole = new (InstalledModules.get("Console")).devConsole();
...
function getWifi() {
    if (navigator.onLine) {
        ...
        document.getElementById("Menu-Wifi-SubStatus").innerHTML = "Not Connected";
    }
